        ...
        if (tag == TAG_PEDIDO) {
            if(origen == mi_cliente){
                ...
                if( (cant_ranks == 2) || (servers_muertos == (cant_ranks/2) -1 ) ){ // No hay otros servers o se murieron todos
                    ...
                    for (server = 0; server < cant_ranks/2; server++){
                        ...
                        if(((server * 2) != mi_rank) && (a_servers_muertos[server] == 0)){
							sprintf(debug_msg, "Pidiendo permiso al server %i con seq: %i", server*2, our_number);
			    			debug(debug_msg);
                ...
                if( buffer > reloj){ 
					...
					sprintf(debug_msg, "Se actualizo el reloj del server %i - nuevo valor: %i", mi_rank /2, reloj);
					debug(debug_msg);
				}
                ...
                if(cliente_en_zona_critica){
					sprintf(debug_msg, "Tengo cliente en zona crítica - dejando pedido de server %i pendiente", origen/2);
					debug(debug_msg);
                    pendientes[origen/2] = 1;
                } else {
                    if(!hay_pedido_local){
                        debug("Dándole permiso a algun server");
						sprintf(debug_msg, "No hay pedido local - Dando permiso al server %i con seq: %i", origen/2 , buffer);
						debug(debug_msg);
                        ...
                        if(buffer == our_number){
                            if(comparacion(mi_rank, origen)){
								sprintf(debug_msg, "Hay pedido local - DIFIRIENDO server %i con seq: %i - MISMO seq que yo", origen / 2, buffer); 
								debug(debug_msg);
                                pendientes[origen/2] = 1;
                            }else{
								sprintf(debug_msg, "Hay pedido local - GRANTING server %i con seq: %i - Menor RANK que yo", origen /2, buffer);
								debug(debug_msg);
                                MPI_Send(NULL, 0, MPI_INT, origen,TAG_OTORGADO, COMM_WORLD);
                            }
                        }else{
                            if(buffer < our_number){
								sprintf(debug_msg, "Hay pedido local - GRANTING server %i con seq: %i - Menor seq que yo", origen /2, buffer);
								debug(debug_msg);
                                MPI_Send(NULL, 0, MPI_INT, origen,TAG_OTORGADO, COMM_WORLD);
                            }else{
								sprintf(debug_msg, "Hay pedido local - DIFIRIENDO server %i con seq:  %i - Yo tengo menor seq", origen /2, buffer);
								debug(debug_msg);
                                pendientes[origen/2] = 1;
                            }
                        }
            ...
            for (iterador = 0; iterador < cant_ranks/2; iterador++){
                if( (pendientes[iterador] == 1) && (iterador*2 != mi_rank) ){
                    pendientes[iterador] = 0;
					sprintf(debug_msg, "Otorgando GRANT a server %i", iterador *2);
					debug(debug_msg);
            ...
            if(origen == mi_cliente){
                ...
                for (iterador = 0; iterador < cant_ranks/2; iterador++){
                    if( (iterador*2 != mi_rank) && (a_servers_muertos[iterador] == 0) ){
						sprintf(debug_msg, "Avisando que TERMINE a server %i", iterador *2);
						debug(debug_msg);
                ...
                servers_muertos++;
                pendientes[origen] = 0;
				a_servers_muertos[origen/2] = 1;
				sprintf(debug_msg, "Se murió el server %i - Cantidad de servers muertos %i", origen / 2, servers_muertos);
				debug(debug_msg);
                if(hay_pedido_local){
					// Hace falta otorgarle agregar el GRANT del origen a esta altura?
					// Debería haber otorgado el GRANT antes del TAG_TERMINE
				}
            }
        }
